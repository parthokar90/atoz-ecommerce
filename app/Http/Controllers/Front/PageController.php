...
class PageController extends Controller
{
    ...
    public function productView($id,$title)
    {
        ...
        $customer_review_count=DB::table('wp_comments')
        ->where('comment_type','product_review')
         ->where('comment_approved',1)
        ->where('comment_post_ID',$id)
        ->count();

        $total_review=DB::table('wp_commentmeta')
        ->where('meta_key','customer_review')
        ->where('comment_post_ID',$id)
        ->where('comment_approved',1)
        ->join('wp_comments','wp_comments.comment_ID','=','wp_commentmeta.comment_id')
        ->sum('meta_value');
        if($customer_review_count!=0){
           $average_review=$total_review/$customer_review_count;
        }else{
             $average_review=0;
        }
        return view('front.productDetails',compact('product','product_related','gallery_images', 'arributeArray',
        'lists','attributes','count','customer_review','customer_review_count','average_review'));
    }
    ...
    public function userProductReview(Request $request){
      $id=Crypt::decrypt($request->product_id);
      ...
      DB::table('wp_commentmeta')->insert([
          'comment_id'=>$last_id,
          'meta_key'=>'customer_review',
          'meta_value'=>$request->review,
      ]);
     return redirect()->back()->with('success', 'Thank You for your review');
    }

    public function subscribeMail(Request $request){
      $count=DB::table('usermeta')->where('meta_key','mail_subscribe')
      ->where('meta_value',$request->user_email)
      ->count();
      if($count>0){
         DB::table('usermeta')->where('meta_key','mail_subscribe')
         ->where('meta_value',$request->user_email)
         ->delete();
         DB::table('usermeta')->insert([
           'user_id'=>'',
           'meta_key'=>'mail_subscribe',
           'meta_value'=>$request->user_email,
         ]);
      }else{
        DB::table('usermeta')->insert([
           'user_id'=>'',
      ...
      return redirect()->back()->with('subscribe_success', 'Thank you');
    }
