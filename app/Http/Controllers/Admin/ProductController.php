...
class ProductController extends Controller
{
    ...
    public function store(ProductStoreRequest $request){
        ...
        $slug = preg_replace('/\s+/', '', $request->post_title);
        $product=array(
            ...
            'post_type'=>'product',
            'guid'=>$slug,
            'post_mime_type'=>$request->featured_product,            
        );        
...
public function stockSkuSearch(Request $request){
   ...
   return view('admin.order.stock_search',compact('products','sku'))->with($extraInfo);
    ...
    public function ReviewList(){
         $customer_review=DB::table('wp_comments')
        ->where('comment_type','product_review')
        ->leftjoin('users','users.id','=','wp_comments.user_id')
        ->leftjoin('posts','posts.ID','=','wp_comments.comment_post_ID')
        ->orderBy('posts.ID','DESC')
        ->paginate(10);
        return view('admin.product.review',compact('customer_review'));
    }

    public function ReviewListAction($id,$status){
      DB::table('wp_comments')
      ->where('comment_type','product_review')
      ->where('comment_ID',$id)
      ->update([
         'comment_approved' =>$status
      ]);
      session()->flash("success","Review has been update successfully");
      return back();
    }
    ...
    public function ReviewListDetails($id){
        $customer_review=DB::table('wp_comments')
        ->where('comment_type','product_review')
        ->where('comment_ID',$id)
        ->leftjoin('users','users.id','=','wp_comments.user_id')
        ->leftjoin('posts','posts.ID','=','wp_comments.comment_post_ID')
        ->first();
        return view('admin.product.review_details',compact('customer_review'));
    }

    public function mail_subscribe_list(){
      $list=DB::table('usermeta')->where('meta_key','mail_subscribe')
      ->paginate(10);
      return view('admin.subscription.list',compact('list'));
    }

    public function mail_subscribe_list_send($id){
       $list=DB::table('usermeta')->where('meta_key','mail_subscribe')->where('umeta_id',$id)
      ->first();
       return view('admin.subscription.send_email',compact('list'));
    }

    public function subscribeMailSend(Request $request){
        // $email = $request->email;
        // $subject=$request->subject;
        // Mail::send('mail',function($message) use ($email, $subject) {
        //      $message->from('@bigshotstyle.com', 'Ecommerce');
